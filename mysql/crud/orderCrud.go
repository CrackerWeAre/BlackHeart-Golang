...
func GetOrderList(query string) []model.Order {
	...
	for rows.Next() {
		err := rows.Scan(&order.OID, &order.UID, &order.OOrderNum, &order.OOrderDate, &order.PID, &order.PName, &order.POption, &order.PPrice, &order.PDistount, &order.OQuantity, &order.ODestination, &order.OPhoneNum, &order.OPayment, &order.OInvoice, &order.ODelivery, &order.OStatus)
		utils.CheckErr(err)
